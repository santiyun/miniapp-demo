...
Page({
	...
	onLoad: function (options)
	{
		...
		try
		{
			...
			// 
			value = wx.getStorageSync('isAutoPush');
			if (value)
			{
				Utils.log(`index page onLoad - getStorageSync(isAutoPush) value: ${value}`);

				isAutoPush = (value === 'true');
				// 
			}
			else
			{
				Utils.log('index page onLoad - getStorageSync(isAutoPush) no-data');
			}
			// 
	...
	onJoin: function ()
	{
		...
		else
		{
			...
			const url = `../meeting/meeting?isCustom=${isCustom}&customServer=${customServer}&appId=${appId}&roomId=${roomId}&userId=${userId}&role=${role}&autoPull=${autoPull}&autoPush=${autoPush}&isTest=${isTest}&logSubmit=${logSubmit}`;
			Utils.log(`*** ${url}`);
	...
	checkboxChangeDemoMode: function (e)
	{
	...
	checkboxChangeAutoPush: function (e)
	{
		let value = e.detail.value.toString();
		let items = this.data.chkAutoPush || [];
		...
		this.setData({ isAutoPush });

		//
		try
		{
			wx.setStorageSync('isAutoPush', isAutoPush ? 'true' : 'false');
		}
		catch (e)
		{
			Utils.log(`checkboxChangeAutoPush failed. e: ${JSON.stringify(e)}`);
		}

		Utils.log(`checkboxChangeAutoPush : ${this.data.isAutoPush}`);
	},

	checkboxChangeAutoPull: function (e)
	{
		let value = e.detail.value.toString();
	...
	checkboxChangeLogSubmit: function (e)
	{
		let value = e.detail.value.toString();
	...
	checkboxChangeCustomServer: function (e)
	{
		let value = e.detail.value.toString();
